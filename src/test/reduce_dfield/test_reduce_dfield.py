...
def test_collection_to_deformation_field_transform():
    ...
    EXPECTED_OUTPUT_SIZE = [
        size / scale for size, scale in zip(INPUT_SIZE, SCALE_FACTORS)
    ]
    ...
    transforms = itk_dreg.reduce_dfield.transform_collection.TransformCollection(
        transform_and_domain_list=[
            itk_dreg.reduce_dfield.transform_collection.TransformEntry(
    ...
    output_transform = (
        itk_dreg.reduce_dfield.transform.collection_to_deformation_field_transform(
            transforms,
    ...
    assert all(
        [
            ...
            for output_size, expected_size in zip(
                itk.size(output_transform.GetDisplacementField()), EXPECTED_OUTPUT_SIZE
            )
        ]
    ), f"Output has size {itk.size(output_transform.GetDisplacementField())}"
    assert np.all(
        itk.array_view_from_image(output_transform.GetDisplacementField())
