...
from itk_dreg.reduce_dfield.transform_collection import (
    TransformEntry,
    TransformCollection,
)
...
def test_unbounded_transform():
    ...
    assert np.all(
        transform_collection.transform_point(itk.Point[itk.F, 3]([0, 0, 0]))
        == [1, 1, 1]
    )
    assert np.all(
        transform_collection.transform_point(itk.Point[itk.F, 3]([5, 10, 15]))
        == [6, 11, 16]
    )
    assert np.all(
        transform_collection.transform_point(itk.Point[itk.F, 3]([-30, -0.1, -0.2]))
        == [-29, 0.9, 0.8]
    )
...
def test_bounded_transform():
    ...
    assert np.all(
        transform_collection.transform_point(itk.Point[itk.F, 3]([1, 1, 1]))
        == [2, 2, 2]
    )
...
def test_two_bounded_transforms():
    transforms = [
        TransformEntry(
            itk.TranslationTransform[itk.D, 3].New(), itk.Image[itk.F, 3].New()
        ),
        TransformEntry(
            itk.TranslationTransform[itk.D, 3].New(), itk.Image[itk.F, 3].New()
        ),
    ...
    assert np.all(
        transform_collection.transform_point(itk.Point[itk.F, 3]([2.4, 2.4, 2.4]))
        == [4.4, 4.4, 4.4]
    )
...
def test_distance_weighted_blending():
    ...
    assert np.all(
        transform_collection.transform_point(itk.Point[itk.F, 3]([4.4, 4.4, 4.4]))
        == [6.4, 6.4, 6.4]
    )
    ...
    assert np.all(
        transform_collection.transform_point(itk.Point[itk.F, 3]([2, 2, 2]))
        == [3.5] * 3
    )
    assert np.all(
        transform_collection.transform_point(itk.Point[itk.F, 3]([3, 3, 3]))
        == [4.75] * 3
    )
    assert np.all(
        transform_collection.transform_point(itk.Point[itk.F, 3]([1, 2, 3]))
        == [2.5, 3.5, 4.5]
    )
