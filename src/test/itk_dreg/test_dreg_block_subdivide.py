...
def test_rescale_physical_region_norescale():
    ...
    physical_region = itk_dreg.block.convert.image_to_physical_region(
        image_region=input_image.GetLargestPossibleRegion(), ref_image=input_image
    )
    output_image = itk_dreg.block.image.physical_region_to_itk_image(
        physical_region=physical_region,
        spacing=[
            spacing * scale
            for spacing, scale in zip(itk.spacing(input_image), SCALE_FACTORS)
        ],
    ...
    assert np.all(
        itk_dreg.block.image.get_sample_bounds(output_image)
...
def test_rescale_physical_region_requested():
    ...
    physical_region = itk_dreg.block.convert.image_to_physical_region(
        image_region=REQUESTED_REGION, ref_image=input_image
    )
    output_image = itk_dreg.block.image.physical_region_to_itk_image(
        physical_region=physical_region,
        spacing=[
    ...
    assert np.all(
        ...
        == itk_dreg.block.convert.image_to_physical_region(
            REQUESTED_REGION, ref_image=input_image
        )
    )
...
def test_rescale_physical_region_with_direction():
    ...
    assert input_image.GetLargestPossibleRegion().IsInside(REQUESTED_REGION)
    assert np.all(
        itk_dreg.block.convert.image_to_physical_region(REQUESTED_REGION, input_image)
        == np.array([[-39.5, 24.5, -39.5], [-24.5, 39.5, -24.5]])
    )
    ...
    EXPECTED_SPACING = SCALE_FACTORS
    EXPECTED_SIZE = [8] * 3
    EXPECTED_PHYSICAL_REGION = np.array([[-40, 24, -40], [-24, 40, -24]])
    ...
    assert np.all(
        itk_dreg.block.image.get_sample_bounds(output_image) == EXPECTED_PHYSICAL_REGION
    )
