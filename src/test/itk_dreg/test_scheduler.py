...
def fixed_image_filepath(test_input_dir):
    ...
    COMPRESSED_FIXED_IMAGE_URL = 'https://data.kitware.com/api/v1/item/65328e0a5be10c8fb6ed4f01/download'
...
def moving_image_filepath(test_input_dir) -> str:
    ...
    MOVING_IMAGE_FILEPATH = os.path.abspath(f'{test_input_dir}/HeadMRVolume_rigid_d.mha')
    COMPRESSED_MOVING_IMAGE_URL = 'https://data.kitware.com/api/v1/item/65328e0f5be10c8fb6ed4f04/download'
...
def test_run_singlethreaded(fixed_image_filepath, moving_image_filepath, test_output_dir):
    ...
    # Methods
    import functools
    fixed_reader_ctor = functools.partial(itk_dreg.itk.make_reader, filepath=fixed_image_filepath)
    moving_reader_ctor = functools.partial(itk_dreg.itk.make_reader, filepath=moving_image_filepath)
    ...
    assert register_method.num_calls == np.product(registration_graph.fixed_da.numblocks)
